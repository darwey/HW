app.controller("LabResultCtrl", ["$scope", "$http", function (s, h) {
    ...
    s.uploadLabResult = async function() {
        swal({
            title: "SAVING",
        ...
        if (fileUpload != undefined) {
            if (fileUpload.size / 1024 / 1024 > 2) {
                ...
                const compressedFile = await compressPdf(fileUpload);

                let formData = new FormData();
                formData.append("file", compressedFile, fileUpload.name);
                formData.append("qrCode", s.labInfo.qrCode);
                formData.append("labID", s.labInfo.labID);

                $.ajax({
                    url: "../LaboratoryResult/UploadPdfResult",
                    type: "POST",
                    data: formData,
                    contentType: false,
                    processData: false,
    ...
    async function compressPdf(file) {
        ...
        // Optionally: Add any compressing logic here
    
        const pdfBytes = await pdfDoc.save({ useObjectStreams: false }); // Disable object streams to reduce file size
